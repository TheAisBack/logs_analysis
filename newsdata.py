...
def query_1():
    ...
    c.execute("SELECT title, count(*) AS views FROM articles, log WHERE " +
              "articles.slug=substring(log.path FROM 10) GROUP BY articles.title " +
...
def query_2():
    ...
    c = db.cursor()
    # execute query_2
    c.execute("SELECT name, count(*) AS views FROM authors, articles, log WHERE " +
              "articles.slug = substring(log.path FROM 10) GROUP BY authors.name " +
...
def query_3():
    ...
    c = db.cursor()
    # execute query_3
    c.execute("SELECT to_char(time, 'FMMonth DD, YYYY'), round(100.0*sum(case log.status " +
              "when '404 NOT FOUND' then 1 else 0 end)/count(log.status), 2) AS error FROM " +
              "log GROUP BY to_char(time, 'FMMonth DD, YYYY') ORDER BY error DESC LIMIT 1")
    # Get the results from the cursor
    results = c.fetchall()
